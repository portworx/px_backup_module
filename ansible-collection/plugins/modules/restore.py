...
def parse_gvk(gvk_string: str) -> Dict[str, str]:
    ...
    """
    parts = gvk_string.split('/')

    if len(parts) == 2:
        # Core resource: "version/kind"
        return {
            "group": "",
            "version": parts[0],
            "kind": parts[1]
        ...
        # Non-core resource: "group/version/kind"
        return {
            "group": parts[0],
            "version": parts[1],
            "kind": parts[2]
...
def build_restore_request(params: Dict[str, Any], module: AnsibleModule, client: PXBackupClient) -> Dict[str, Any]:
    ...
    if params.get('filter'):
        ...
        if 'namespace_filter' in filter_param and filter_param['namespace_filter'] is not None:
